...
def importSubstructure(modelData,geometryName,allSubInfo,numOfVars,numOfAtt,pathName):#,var1set,var2set,pathName):
    '''
    ...
    var1set, var2set : LISTS
        Design variable for the substructure to iterate over. For example,
        with the circle substructures, this could be thickness
    pathName : STR
    ...
    subInfoList = [[], [], [], [], []] # list of lists [[STR],[INT],[STR],[ARRAYS of FLOATS],[ARRAYS of FLOATS]]
    subInfoList[0] = substructureInfo[0].astype(int).tolist()
    for name in range(substructureInfo[0].size):#len(subInfoList[0])):
        temp = subInfoList[0][name]
        # print(temp)
        subInfoList[0][name] = substructureRef[temp]
    subInfoList[1] = substructureInfo[1].astype(int).tolist()
    subInfoList[2] = substructureInfo[2].astype(int).tolist()
    for name in subInfoList[2]:name = materialRef[name]
    # print(subInfoList[0])
    # print(subInfoList[1])
    # print(subInfoList[2])
    # print(substructureInfo[3])
    subInfoList[3] = substructureInfo[np.arange(3, 3 + numOfVars)[:, None], np.arange(substructureInfo[3].size)].tolist()
    # print(subInfoList[3])
    subInfoList[4] = substructureInfo[np.arange(3 + numOfVars,3 + numOfVars + numOfAtt)[:, None], np.arange(substructureInfo[4].size)].tolist()
    # print(subInfoList[4])
        
    ####
    modelName = modelData['modelName']
    
    for lenList in range(len(subInfoList[0])):
        partName = subInfoList[0][lenList]+'-'+str(subInfoList[1][lenList])+'_Z'+str(subInfoList[1][lenList])
        jobName = subInfoList[0][lenList]+'-'+str(subInfoList[1][lenList])+'.odb'
        substructureName=subInfoList[0][lenList]+'-'+str(subInfoList[1][lenList])+'_Z'+str(subInfoList[1][lenList])+'.sim'
        ...
        mdb.models[modelName].PartFromSubstructure(name=partName, 
            substructureFile=substructureFileName, 
            odbFile=odbName)
    ...
    for row in range(5):
        allSubInfo[row].extend(subInfoList[row])
       
    return allSubInfo#np.append(allSubInfo,substructureInfo,axis=1)
...
def nearestMatch(desiredAttributes, partsInfo, desiredComp, partsInfoIndex): #how should mass be handled
    ...
    partsInfo : list of Numpy ARRAYS [[STR],[INT],[STR],[ARRAYS of FLOATS],[ARRAYS of FLOATS]]
        Contains all substructure's names, ID, Materials, array of design variables, array of attribute metrics
    ...
    # Input design variable ranges for accurate mapping 
    #thickness = 0.1, 0.8 
    thicknessMin = 0.1
    thicknessMax = 0.8 
    #fillets = 0.1, 2.0 
    ...
    # map thickness DV 
    dAttOrig[:,0] = (thicknessMax-thicknessMin)*dAttOrig[:,0] + thicknessMin 
    #map fillet DV
    dAttOrig[:,1] = (filletMax-filletMin)*dAttOrig[:,1]+filletMin
    print(dAttOrig)
...
def runAssembly():
    ...
    #    allSubInfo=partsInfo,numOfVars=2,numOfAtt=4,pathName=path)
